...
数字颜色：**灰白＝待开始，绿色＝计时中，琥珀黄＝暂停，蓝色 `00:00`＝完成**。顶部左侧组合显示阶段和本轮次数，右上角显示电量百分比（每 5 秒更新，低于或等于 20% 显示琥珀黄，读取失败显示 `--%`）。正在充电时，电量左侧显示黄色闪电，停止充电后隐藏（最多延迟 5 秒更新）。电量由电压估算，充电或负载变化时可能波动。

| 设置 | 范围 | 默认 |
|---|---|---|
| Focus | 5–60 分钟，步进 5 | 25 |
| Short break | 1–15 分钟，步进 1 | 5 |
| Long break | 5–30 分钟，步进 5 | 15 |
| Long interval | 2–6 次 | 4 |
| Sound | On / Off | On |

Settings 列表分为三页，每页最多 3 行，右侧显示当前值。选择 **Reset** 可恢复默认的 25/5/15/4/Sound On，并使用现有保存机制立即保存。设置确认后保存，重启保留设置但重置计时和次数。出现 `Save failed: RAM only` 时，新设置仅在本次运行生效。

30 秒无操作后屏幕由 60% 调暗至约 5%；按键或到时恢复亮度。到时以 70% 音量播放一次内嵌提示音，可在设置中关闭。
